...
class VideosRestControllerTest {
    ...
    void getVideos() {
        ...
        assertTrue(response1.getVideoUrl().endsWith("/media/video1.mp4"));
        assertTrue(response1.getThumbnailUrl().endsWith("/media/thumb1.webp"));

        VideoResponse response2 = body.get(1);
        assertEquals("video 2", response2.getTitle());
        assertEquals("user2", response2.getUser());
        assertTrue(response2.getVideoUrl().endsWith("/media/video2.mp4"));
        assertTrue(response2.getThumbnailUrl().endsWith("/media/thumb2.webp"));
    }
}