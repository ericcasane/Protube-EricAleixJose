...
export default function ExplorePage() {
        ...
        const fetchVideos = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    const mappedVideos: VideoCardProps[] = Array.isArray(data) ? data.map((v: any) => ({
                        id: v.videoId || v.id,
                        title: v.title,
    ...
    const sortedByDate = [...videos].sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
                            ...
                            <div className="absolute bottom-0 left-0 p-8 md:p-12 w-full md:w-2/3 flex flex-col gap-4">
                                ...
                                <div className="flex items-center gap-4 text-white/80">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-semibold">{spotlightVideo.channelName}</span>
                                    </div>
                                    <span className="w-1 h-1 rounded-full bg-white/50" />
                                    <div className="flex items-center gap-2">
                                        <Icon icon="solar:eye-bold" className="text-xl" />
                                        <span>{formatViews(spotlightVideo.viewCount)} {t('video.viewsLabel')}</span>
                                    </div>
                                </div>
                    ...
                    >
                        <div className="flex items-center justify-between">
                            <h3 className="text-2xl font-bold flex items-center gap-2">
                                <Icon icon="solar:fire-bold" className="text-orange-500" />
                                {t('explore.trending')}
                            </h3>
                        </div>
